...
# Plot the theoretical variance from the closed formula given in ()



# Plot the theoretical variance as computed by quadrature from the closed formula of the F-P distribution given in ()
...
hist!(ax2, Et[:,31], bins = 100, color = :red, strokecolor = :black, strokewidth = 1)
ax3 = Axis(fig2[1,2],
    title = L"y=2.00"
)
...
hist!(ax4, Et[:,77], bins = 100, color = :red, strokecolor = :black, strokewidth = 1)
ax5 = Axis(fig2[1,4],
    title = L"y=2.75"
)
hist!(ax5, Et[:,92], bins = 100, color = :red, strokecolor = :black, strokewidth = 1)
...
# Export the figures
save("../results/MultTransEnsemble.png", fig1)
save("../results/MultTransSamplePaths.png", fig2)
